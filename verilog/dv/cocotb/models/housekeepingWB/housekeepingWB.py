...
class HK_whiteBox:
    ...
    def Monitors(self):
        ...
        # wishbone
        wishbone_mon_i = HKmonitor(
            f"HKinputsMonitorwishbone",
            self.hk_hdl,
            inputs["wishbone"],
            self.clk,
            self.reset,
            self.logger,
            callback=self.wb_models.wishbone_model,
        )
        wishbone_mon_o = HKmonitor(
            f"HKoutputsMonitorwishbone",
            self.hk_hdl,
            outputs["wishbone"],
            self.clk,
            self.reset,
        ...
        # UART
        UART_mon_i = HKmonitor(
            f"HKinputsMonitorUART",
            self.hk_hdl,
            inputs["UART"],
            self.clk,
            self.reset,
            self.logger,
            callback=self.wb_models.UART_model,
        )
        UART_mon_o = HKmonitor(
            f"HKoutputsMonitorUART",
            self.hk_hdl,
            outputs["UART"],
            self.clk,
            self.reset,
        ...
        # debug
        debug_mon_i = HKmonitor(
            f"HKinputsMonitordebug",
            self.hk_hdl,
            inputs["debug"],
            self.clk,
            self.reset,
            self.logger,
            callback=self.wb_models.debug_model,
        )
        debug_mon_o = HKmonitor(
            f"HKoutputsMonitordebug",
            self.hk_hdl,
            outputs["debug"],
            self.clk,
            self.reset,
        ...
        # SPI
        SPI_mon_i = HKSPImonitor(
            f"HKinputsMonitorSPI",
            self.hk_hdl,
            inputs["SPI"],
            self.clk,
            self.reset,
            self.logger,
            callback=self.wb_models.spi_model,
        )
        SPI_mon_o = HKSPImonitor(
            f"HKoutputsMonitorSPI",
            self.hk_hdl,
            outputs["SPI"],
            self.clk,
            self.reset,
        ...
        CSBmonitor(
            ...
            self.reset,
            False,
            callback=self.wb_models.reset_spi_vals,
        )
        if self.checkers:
                ...
                for reg_shift, reg in memory_block.items():
                    for field in reg:
                        ...
                        if RTL_reg_name == None:
                            cocotb.log.debug(
                                f"[HK_whiteBox][reg_model_sb] register {field[1]} in {key} doesn't have a RTL register"
                            )
                            continue
                        if isinstance(field[1], list):
                            ...
                            if RTL_name in ["serial_xfer"]:
                                continue  # TODO: change with SDF only
                            if Macros["GL"]:
                                if RTL_name in [
                                    "mfgr_id",
                                    "prod_id",
                                    ...
                                    continue  # TODO: change with SDF only
                            if Macros["GL"]:
                                if fnmatch.fnmatch(RTL_name, "gpio_configure*"):
                                    continue  # TODO: update gpio_configure and mgmt_gpio_data to get each bit in the SDF case
                            ...
                            if field[1] in ["pwr_ctrl_out", "serial_xfer"]:
                                continue  # TODO: delete when reset value is spicified
                            RTL_reg_path = self.hk_hdl._id(field[1], False)
                            RTL_reg_val = RTL_reg_path.value
                        if RTL_reg_val.integer != field[6]:
                            cocotb.log.error(
                                f"[HK_whiteBox][reg_model_sb] mismatch in register {field[1]} in {key} expected val = {int(field[6])} actual val = {int(RTL_reg_val.binstr,2)} "
                            )
                        else:
                            cocotb.log.debug(
                                f"[HK_whiteBox][reg_model_sb] match in register {field[1]} in {key} expected val = {field[6]} actual val = {RTL_reg_val.integer} "
                            )
