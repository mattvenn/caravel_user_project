...
class HK_models:
    def __init__(self, reg_model, expeceted_output, hk_hdl):
        ...
        self.exp_out_uart_rx = [
            self.expeceted_output["UART"]
        ]  # expected output for uart
    ...
    def wishbone_model(self, trans):
        ...
        if trans["stb"]["val"] == 1:  # valid data
            ...
            if trans["write_en"]["val"] == 1:  # write cycle
                cocotb.log.debug(
                    f"[HK_models][_valid_address] write enable for address: {hex(address)} and data: {hex(data)} "
                )
                select = trans["sel"]["val"]
                cocotb.scheduler.add(
                    self._write_reg_wb(address, data, select, clock_signal)
                )
            else:  # read cycle
    ...
    def UART_model(self, trans):
        ...
        if trans["enable"]["val"]:
            ...
            self.gpio_out_pre[6] = int(
                gpio_data[len(gpio_data) - 1 - 6]
            )  # mgmt_gpio_data[6]
        ...
        output["RX"]["val"] = 0
        if trans["enable"]["val"]:
            output["RX"]["val"] = trans["gpio_in"]["val"][
                gpio_in_size - 5
            ]  # gpio_in[5]
    ...
    def debug_model(self, trans):
        ...
        if trans["enable"]["val"]:
            ...
            gpio_data = self._read_reg_keys(["GPIO", "0x0c"])  # mgmt_gpio_data[7:0]
            self.gpio_out_pre[0] = int(
                gpio_data[len(gpio_data) - 1 - 0]
            )  # mgmt_gpio_data[6]
            gpio_data_en = self._read_reg_keys(
                ["GPIO", "0x24"]
            )  # gpio_configure[0][7:0]
            self.mgmt_gpio_oeb[0] = 1 - int(
                gpio_data[len(gpio_data_en) - 1 - self.input_dis]
            )  # gpio_configure[0][3]
        ...
        output["data"]["val"] = 0
        if trans["enable"]["val"]:
            output["data"]["val"] = trans["gpio_in"]["val"][
                gpio_in_size - 0
            ]  # mgmt_gpio_in[0]
    ...
    def spi_model(self, trans):
        ...
        output["SDO"]["val"] = 0
        cocotb.log.debug(
            f"[HK_models][spi_model] spi mode {self.spi_mode[0]} bit number {self.spi_mode[1]} command = {self.command_spi} address = {self.address_spi} write = {self.write_spi} stream = {self.spi_mode[2]}"
        )
    ...
    def _setWriteData(self, bit):
        if bit not in ["0", "1"]:
            cocotb.log.debug(
                f"[HK_models][_setWriteData] incorrect bit size bit = {bit}"
            )
        ...
        if self.spi_mode[1] >= 8:
            ...
            if is_valid:
                ...
                cocotb.log.debug(
                    f"[HK_models][_valid_address] writing {data} to memory:{keys[0]} field: {keys[1]} through housekeeping SPI address = {address}"
                )
    ...
    def _getReadData(self):
        ...
        if self.spi_mode[1] >= 8:
            ...
            self.spi_mode[2] += 1  # stream number
            if self.command_spi[2:5] != ["0", "0", "0"]:
                self.spi_mode_cov("read-n")
            else:
                self.spi_mode_cov("read")
    ...
    async def _write_reg_wb(self, address, data, select, clk):
        ...
        if (
            address == 0x2600000C
        ):  # mgmt_gpio_data is a special case as it got written completely at the end of the serial writing
            for i in range(6):
        ...
        for i, sel in enumerate(select.binstr):
            ...
            if sel is "1":
                ...
                if is_valid:
                    ...
                    cocotb.log.debug(
                        f"[HK_models][_valid_address] writing {data[(8*((4-i-1))):8*(4-i)-1]} to memory:{keys[0]} field: {keys[1]}"
                    )
                    temp_data = data[(8 * ((4 - i - 1))) : 8 * (4 - i) - 1]
                    self._write_fields(keys, temp_data)
                cocotb.log.debug(
                    f" [HK_models][_valid_address] address {hex(address)} used for access housekeeping memory success"
                )
    ...
    def _write_fields(self, keys, data):
        for field in self.reg_model[keys[0]][keys[1]]:
            if field[4] == "RW":
                ...
                size = field[3]
                cocotb.log.debug(
                    f"[HK_models][_write_fields] before update field : {field[1]} data = {bin(field[6])} with data {data[shift:shift+size-1]} "
                )
                field[6] = data[8 - shift - size : 8 - shift - 1]
                cocotb.log.debug(
                    f"[HK_models][_write_fields] after update field : {field[1]} data = {bin(field[6])} with data {data[shift:shift+size-1]} "
                )
        ...
        for i in range(4):
            ...
            if is_valid:
                ...
                for field in self.reg_model[keys[0]][keys[1]]:
                    ...
                    data = bin(field[6])[2:].zfill(size)
                    cocotb.log.debug(
                        f"[HK_models][_read_reg] memory:{keys[0]} shift:{keys[1]} field:{field[0]} data:{data} "
                    )
                    for k, j in enumerate(range(first_index, second_index, 1)):
                        data_out[j] = data[k]
                    data_string = "".join(data_out)
        cocotb.log.debug(
            f"[HK_models][_read_reg] register {hex(address)} has value  {hex(int(data_string,2))} "
        )
    ...
    def _valid_address(self, address):
        ...
        for key, memory in self.reg_model.items():
            ...
            cocotb.log.debug(
                f'[HK_models][_valid_address] base address[:13]={hex(int(base_addr[size-32:size-13],2))} target address[:13]={hex(int(address[size-32:size-13],2))} shift={"{0:#0{1}x}".format(int(address[size-12:],2),4)}'
            )
            if base_addr[size - 32 : size - 13] == address[size - 32 : size - 13]:
                if "{0:#0{1}x}".format(int(address[size - 12 :], 2), 4) in memory:
                    cocotb.log.debug(
                        f"[HK_models][_valid_address] base address={hex(int(base_addr,2))} key {key}"
                    )
                    return True, [
                        key,
                        "{0:#0{1}x}".format(int(address[size - 12 :], 2), 4),
                    ]
        cocotb.log.debug(
            f"[HK_models][_valid_address] address {hex(int(address,2))} used for access housekeeping memory isn't valid"
        )
        return False, None
    ...
    def _valid_address_spi(self, address):
        ...
        cocotb.log.debug(
            f"[HK_models][_valid_address_spi] address {hex(address)} for SPI housekeeping isn't valid"
        )
        return False, None
    ...
    def reg_cov(self, key1, key2, is_SPI=True, is_read=True):
        ...
        @CoverPoint(
            f"top.caravel.housekeeping.regs.{s}",
    ...
    def spi_mode_cov(self, mode):
        @CoverPoint(
            f"top.caravel.housekeeping.spi.modes",
            bins=[
                "noOP",
                "write",
                "read",
            ...
            bins_labels=[
                ...
                "Simultaneous Read/Write in streaming mode",
                "Write in n-byte mode",
                "Read in n-byte mode",
                "Simultaneous Read/Write in n-byte mode",
                "Pass-through (management) Read/Write streaming mode",
