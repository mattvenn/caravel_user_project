...
class GPIO_ctrlWB:
    ...
    def setupModels(self):
        self.gpio_model = GPIO_models(
            f"{self.gpio_name}",
            serial_out_if=copy.deepcopy(self.serial_if["outputs"]),
    ...
    def Monitors(self):
        DefaultMonitor(
            f"{self.gpio_name}",
            self.gpio_hdl,
            self.default_if["inputs"],
            self.clk,
            self.reset,
            callback=self.gpio_model.default_model,
        )
        SerialMonitor(
            f"{self.gpio_name}_inputs",
            self.gpio_hdl,
            self.serial_if["inputs"],
            self.clk,
            self.reset,
            callback=self.gpio_model.serial_model,
        )
        self.serial_monitor_o = SerialMonitor(
            f"{self.gpio_name}_outputs",
            self.gpio_hdl,
            self.serial_if["outputs"],
            self.clk,
            self.reset,
            is_output=True,
        )
        PadMonitor(
            f"{self.gpio_name}_inputs",
            self.gpio_hdl,
            self.pad_gpio_if["inputs"],
            self.clk,
            self.reset,
            callback=self.gpio_model.pad_in_model,
        )
        PadMonitor(
            f"{self.gpio_name}_outputs",
            self.gpio_hdl,
            self.pad_gpio_if["outputs"],
            self.clk,
            self.reset,
            is_output=True,
            callback=self.gpio_model.pad_out_model,
        )
        MgmtGpioMonitor(
            f"{self.gpio_name}_inputs",
            self.gpio_hdl,
            self.mgmt_gpio_if["inputs"],
            self.clk,
            self.reset,
            callback=self.gpio_model.mgmt_gpio_model,
        )
        MgmtGpioMonitor(
            f"{self.gpio_name}_outputs",
            self.gpio_hdl,
            self.mgmt_gpio_if["outputs"],
            self.clk,
            self.reset,
            is_output=True,
            callback=self.gpio_model.gpio_mgmt_model,
        )
        UserGpioMonitor(
            f"{self.gpio_name}_inputs",
            self.gpio_hdl,
            self.user_gpio_if["inputs"],
            self.clk,
            self.reset,
            callback=self.gpio_model.user_gpio_model,
        )
        UserGpioMonitor(
            f"{self.gpio_name}_outputs",
            self.gpio_hdl,
            self.user_gpio_if["outputs"],
            self.clk,
            self.reset,
            is_output=True,
            callback=self.gpio_model.gpio_user_model,
        )
